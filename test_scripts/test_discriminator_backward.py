...
class ToyDiscriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        out = flow.sigmoid(self.linear(feat))
        return out
...
def main():
    ...
    flow.manual_seed(manual_seed)

    img_real = flow.rand((1, 3, 6, 6))
    img_fake = flow.rand((1, 3, 6, 6))
    ...
    # https://pyflow.org/tutorials/beginner/dcgan_faces_tutorial.html
    ###########################
    net_d_1.zero_grad()
    # real
    ...
    for k1, k2 in zip(net_d_1.parameters(), net_d_2.parameters()):
        print(flow.sum(flow.abs(k1.grad - k2.grad)))


if __name__ == "__main__":
    main()
r"""Output:
conv0.weight flow.Size([4, 3, 3, 3])
conv0.bias flow.Size([4])
bn0.weight flow.Size([4])
bn0.bias flow.Size([4])
conv1.weight flow.Size([4, 4, 3, 3])
conv1.bias flow.Size([4])
bn1.weight flow.Size([4])
bn1.bias flow.Size([4])
linear.weight flow.Size([1, 144])
linear.bias flow.Size([1])
tensor(0.)
tensor(0.)
tensor(0.)
tensor(0.)
