...
def convert_edvr():
    ori_net = flow.load("experiments/pretrained_models/EDVR_REDS_SR_M.pth")
    crt_net = flow.load("xxx/net_g_8.pth")
    ...
    flow.save(crt_net, save_path)


def convert_edsr(ori_net_path, crt_net_path, save_path, num_block=32):
    """Convert EDSR models in https://github.com/thstkdgus35/EDSR-Pyflow.
    ...
    """
    ori_net = flow.load(ori_net_path)
    crt_net = flow.load(crt_net_path)
...
def convert_rcan_model():
    ori_net = flow.load("RCAN_model_best.pt")
    crt_net = flow.load(
    ...
    flow.save(crt_net, "RCAN_model_best.pth")


def convert_esrgan_model():
    from onesr.archs.rrdbnet_arch import RRDBNet
    ...
    ori_net = flow.load("experiments/pretrained_models/RRDB_ESRGAN_x4.pth")
...
def convert_duf_model():
    from onesr.archs.duf_arch import DUF
    ...
    ori_net = flow.load("experiments/pretrained_models/old_DUF_x2_16L_official.pth")
    # print('******')
    # for k, v in ori_net.items():
    #     print(k)
    """
    ...
    flow.save(crt_net, "experiments/pretrained_models/DUF_x2_16L_official.pth")
