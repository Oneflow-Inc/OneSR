...
def calculate_stylegan2_fid():
    ...
    generator.load_state_dict(flow.load(args.ckpt)["params_ema"])
    ...
    def sample_generator(total_batch):
        for _ in range(total_batch):
            with flow.no_grad():
                latent = flow.randn(args.batch_size, 512, device=device)
                samples, _ = generator(
                    [latent],
    ...
    stats = flow.load(args.fid_stats)
