...
from onesr.metrics import calculate_psnr, calculate_ssim
from onesr.utils import bgr2ycbcr, scandir
...
def main(args):
    ...
    img_list_restored = sorted(
        list(scandir(args.restored, recursive=True, full_path=True))
    )
    ...
    print(args.restored)
    print(
        f"Average: PSNR: {sum(psnr_all) / len(psnr_all):.6f} dB, SSIM: {sum(ssim_all) / len(ssim_all):.6f}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gt",
        type=str,
        default="datasets/val_set14/Set14",
        help="Path to gt (Ground-Truth)",
    )
    parser.add_argument(
        "--restored", type=str, default="results/Set14", help="Path to restored images"
    )
    parser.add_argument(
        "--crop_border", type=int, default=0, help="Crop border for each side"
    )
    parser.add_argument(
        "--suffix", type=str, default="", help="Suffix for restored images"
    )
    parser.add_argument(
        "--test_y_channel",
        action="store_true",
