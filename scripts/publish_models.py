...
from oneflow.serialization import _is_zipfile, _open_file_like
...
def update_sha(paths):
    ...
    for idx, path in enumerate(paths):
        ...
        net = flow.load(path, map_location=flow.device("cpu"))
...
def convert_to_backward_compatible_models(paths):
    ...
    PyTorch 1.6 uses a updated version of flow.save. In order to be compatible
    ...
    for idx, path in enumerate(paths):
        ...
        if flag_need_conversion:
            ...
            flow.save(net, path, _use_new_zipfile_serialization=False)
