...
from onesr.archs.swinir_arch import SwinIR
...
def main():
    ...
    parser.add_argument(
        "--input",
        type=str,
        default="datasets/Set5/LRbicx4",
        help="input test image folder",
    )
    parser.add_argument(
        "--output", type=str, default="results/SwinIR/Set5", help="output folder"
    )
    parser.add_argument(
        "--task",
        type=str,
    ...
    parser.add_argument(
        "--patch_size", type=int, default=64, help="training patch size"
    )
    parser.add_argument(
        "--scale", type=int, default=4, help="scale factor: 1, 2, 3, 4, 8"
    ...
    parser.add_argument(
        "--large_model",
        action="store_true",
        help="Use large model, only used for real image sr",
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(args.input, "*")))):
        ...
        img = flow.from_numpy(np.transpose(img[:, :, [2, 1, 0]], (2, 0, 1))).float()
        ...
        with flow.no_grad():
            ...
            _, _, h, w = output.size()
            output = output[
                :, :, 0 : h - mod_pad_h * args.scale, 0 : w - mod_pad_w * args.scale
            ]
...
def define_model(args):
    ...
    loadnet = flow.load(args.model_path)
