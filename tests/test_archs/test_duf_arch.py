...
from onesr.archs.duf_arch import DUF, DynamicUpsamplingFilter
...
def test_duf():
    ...
    img = flow.rand((1, 7, 3, 48, 48), dtype=flow.float32).cuda()
    output = net(img)
...
def test_dynamicupsamplingfilter():
    ...
    img = flow.rand((2, 3, 12, 12), dtype=flow.float32).cuda()
    filters = flow.rand((2, 9, 2, 12, 12), dtype=flow.float32).cuda()
