...
from onesr.archs.vgg_arch import VGGFeatureExtractor
...
def charbonnier_loss(pred, target, eps=1e-12):
    return flow.sqrt((pred - target) ** 2 + eps)
...
class PerceptualLoss(nn.Module):
    ...
    def __init__(
        ...
        if self.criterion_type == "l1":
            ...
            self.criterion = flow.nn.MSELoss()
        ...
        if self.perceptual_weight > 0:
            ...
            for k in x_features.keys():
                if self.criterion_type == "fro":
                    percep_loss += (
                        flow.norm(x_features[k] - gt_features[k], p="fro")
    ...
    def _gram_mat(self, x):
        ...
        Args:
            x (flow.Tensor): Tensor with shape of (n, c, h, w).

        Returns:
            flow.Tensor: Gram matrix.
        """
        n, c, h, w = x.size()
