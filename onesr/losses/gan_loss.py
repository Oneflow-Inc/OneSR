...
from onesr.utils.registry import LOSS_REGISTRY
...
def g_path_regularize(fake_img, latents, mean_path_length, decay=0.01):
    noise = flow.randn_like(fake_img) / math.sqrt(fake_img.shape[2] * fake_img.shape[3])
    grad = autograd.grad(
        outputs=(fake_img * noise).sum(), inputs=latents, create_graph=True
    )[0]
    path_lengths = flow.sqrt(grad.pow(2).sum(2).mean(1))
...
def gradient_penalty_loss(discriminator, real_data, fake_data, weight=None):
    ...
    alpha = real_data.new_tensor(flow.rand(batch_size, 1, 1, 1))
    ...
    gradients = autograd.grad(
        ...
        inputs=interpolates,
        grad_outputs=flow.ones_like(disc_interpolates),
        create_graph=True,
        retain_graph=True,
        only_inputs=True,
    )[0]
    ...
    if weight is not None:
        gradients_penalty /= flow.mean(weight)
