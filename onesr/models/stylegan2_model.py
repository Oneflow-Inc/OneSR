...
from onesr.losses.gan_loss import g_path_regularize, r1_penalty
...
class StyleGAN2Model(BaseModel):
    ...
    def make_noise(self, batch, num_noise):
        if num_noise == 1:
            noises = flow.randn(batch, self.num_style_feat, device=self.device)
        else:
            noises = flow.randn(
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        if self.opt["is_train"]:
            save_img_path = osp.join(
                self.opt["path"]["visualization"], "train", f"train_{current_iter}.png"
            )
        else:
            save_img_path = osp.join(
