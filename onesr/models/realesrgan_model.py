...
from onesr.data.degradations import (
    random_add_gaussian_noise_pt,
    random_add_poisson_noise_pt,
)
from onesr.data.transforms import paired_random_crop
from onesr.losses.loss_util import get_refined_artifact_map
from onesr.models.srgan_model import SRGANModel
from onesr.utils import DiffJPEG, USMSharp
from onesr.utils.img_process_util import filter2D
from onesr.utils.registry import MODEL_REGISTRY


@MODEL_REGISTRY.register(suffix="onesr")
class RealESRGANModel(SRGANModel):
    ...
    def _dequeue_and_enqueue(self):
        ...
        if not hasattr(self, "queue_lr"):
            ...
            self.queue_lr = flow.zeros(self.queue_size, c, h, w).cuda()
            _, c, h, w = self.gt.size()
            self.queue_gt = flow.zeros(self.queue_size, c, h, w).cuda()
            self.queue_ptr = 0
        if self.queue_ptr == self.queue_size:  # the pool is full
            # do dequeue and enqueue
            # shuffle
            idx = flow.randperm(self.queue_size)
    ...
    def feed_data(self, data):
        ...
        if self.is_train and self.opt.get("high_order_degradation", True):
            ...
            # clamp and round
            self.lq = flow.clamp((out * 255.0).round(), 0, 255) / 255.0
    ...
    def optimize_parameters(self, current_iter):
        ...
        if (
            ...
            if self.cri_ldl:
                ...
                l_g_ldl = self.cri_ldl(
                    flow.mul(pixel_weight, self.output), flow.mul(pixel_weight, self.gt)
                )
                l_g_total += l_g_ldl
        ...
        loss_dict["out_d_real"] = flow.mean(real_d_pred.detach())
        l_d_real.backward()
        # fake
        fake_d_pred = self.net_d(self.output.detach().clone())  # clone for pt1.9
        l_d_fake = self.cri_gan(fake_d_pred, False, is_disc=True)
        loss_dict["l_d_fake"] = l_d_fake
        loss_dict["out_d_fake"] = flow.mean(fake_d_pred.detach())
        l_d_fake.backward()
