...
from onesr.archs import build_network
from onesr.losses import build_loss
from onesr.metrics import calculate_metric
...
class HiFaceGANModel(SRModel):
    ...
    def discriminate(self, input_lq, output, ground_truth):
        ...
        if output.shape[-2:] != input_lq.shape[-2:]:
            lq = flow.nn.functional.interpolate(input_lq, (h, w))
            real = flow.nn.functional.interpolate(ground_truth, (h, w))
            fake_concat = flow.cat([lq, output], dim=1)
            real_concat = flow.cat([lq, real], dim=1)
        else:
            fake_concat = flow.cat([input_lq, output], dim=1)
            real_concat = flow.cat([input_lq, ground_truth], dim=1)

        fake_and_real = flow.cat([fake_concat, real_concat], dim=0)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        if with_metrics:
            sr_pack = flow.cat(sr_tensors, dim=0)
            gt_pack = flow.cat(gt_tensors, dim=0)
            # calculate metrics
