...
from onesr.metrics.psnr_ssim import calculate_psnr_pt, calculate_ssim_pt
from onesr.utils import img2tensor
...
def test(img_path, img_path2, crop_border, test_y_channel=False):
    ...
    psnr_pth = calculate_psnr_pt(
        img, img2, crop_border=crop_border, test_y_channel=test_y_channel
    )
    ...
    psnr_pth = calculate_psnr_pt(
        flow.repeat_interleave(img, 2, dim=0),
        flow.repeat_interleave(img2, 2, dim=0),
        crop_border=crop_border,
        test_y_channel=test_y_channel,
    )
    ssim_pth = calculate_ssim_pt(
