...
from onesr.metrics.metric_util import reorder_image, to_y_channel
from onesr.utils.color_util import rgb2ycbcr_pt
from onesr.utils.registry import METRIC_REGISTRY
...
def calculate_psnr_pt(img, img2, crop_border, test_y_channel=False, **kwargs):
    ...
    img = img.to(flow.float64)
    img2 = img2.to(flow.float64)

    mse = flow.mean((img - img2) ** 2, dim=[1, 2, 3])
    return 10.0 * flow.log10(1.0 / (mse + 1e-8))
