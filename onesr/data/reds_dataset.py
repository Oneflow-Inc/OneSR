...
from oneflow.utils import data as data

from onesr.data.transforms import augment, paired_random_crop
from onesr.utils import FileClient, get_root_logger, imfrombytes, img2tensor
from onesr.utils.flow_util import dequantize_flow
from onesr.utils.registry import DATASET_REGISTRY
...
class REDSDataset(data.Dataset):
    ...
    onesr/data/meta_info/meta_info_REDS_GT.txt
    ...
    def __getitem__(self, index):
        ...
        img_lqs = flow.stack(img_results[0:-1], dim=0)
        img_gt = img_results[-1]

        if self.flow_root is not None:
            img_flows = img2tensor(img_flows)
            # add the zero center flow
            img_flows.insert(self.num_half_frames, flow.zeros_like(img_flows[0]))
            img_flows = flow.stack(img_flows, dim=0)
...
class REDSRecurrentDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_results = img2tensor(img_results)
        img_gts = flow.stack(img_results[len(img_lqs) // 2 :], dim=0)
        img_lqs = flow.stack(img_results[: len(img_lqs) // 2], dim=0)
