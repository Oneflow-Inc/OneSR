...
from flowvision.transforms.functional_tensor import rgb_to_grayscale
...
def generate_gaussian_noise_pt(img, sigma=10, gray_noise=0):
    ...
    if isinstance(gray_noise, (float, int)):
        ...
        cal_gray_noise = flow.sum(gray_noise) > 0

    if cal_gray_noise:
        noise_gray = (
            flow.randn(*img.size()[2:4], dtype=img.dtype, device=img.device)
            * sigma
            / 255.0
        )
    ...
    noise = flow.randn(*img.size(), dtype=img.dtype, device=img.device) * sigma / 255.0
...
def add_gaussian_noise_pt(img, sigma=10, gray_noise=0, clip=True, rounds=False):
    ...
    out = img + noise
    if clip and rounds:
        out = flow.clamp((out * 255.0).round(), 0, 255) / 255.0
    elif clip:
...
def random_generate_gaussian_noise_pt(img, sigma_range=(0, 10), gray_prob=0):
    sigma = (
        flow.rand(img.size(0), dtype=img.dtype, device=img.device)
        * (sigma_range[1] - sigma_range[0])
        + sigma_range[0]
    )
    gray_noise = flow.rand(img.size(0), dtype=img.dtype, device=img.device)
...
def generate_poisson_noise_pt(img, scale=1.0, gray_noise=0):
    ...
    if cal_gray_noise:
        ...
        # round and clip image for counting vals correctly
        img_gray = flow.clamp((img_gray * 255.0).round(), 0, 255) / 255.0
        # use for-loop to get the unique values for each sample
        vals_list = [len(flow.unique(img_gray[i, :, :, :])) for i in range(b)]
        vals_list = [2 ** np.ceil(np.log2(vals)) for vals in vals_list]
        vals = img_gray.new_tensor(vals_list).view(b, 1, 1, 1)
        out = flow.poisson(img_gray * vals) / vals
        noise_gray = out - img_gray
        noise_gray = noise_gray.expand(b, 3, h, w)

    # always calculate color noise
    # round and clip image for counting vals correctly
    img = flow.clamp((img * 255.0).round(), 0, 255) / 255.0
    # use for-loop to get the unique values for each sample
    vals_list = [len(flow.unique(img[i, :, :, :])) for i in range(b)]
    vals_list = [2 ** np.ceil(np.log2(vals)) for vals in vals_list]
    vals = img.new_tensor(vals_list).view(b, 1, 1, 1)
    out = flow.poisson(img * vals) / vals
    noise = out - img
    if cal_gray_noise:
        noise = noise * (1 - gray_noise) + noise_gray * gray_noise
