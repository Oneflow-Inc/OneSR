...
class Vimeo90KDataset(data.Dataset):
    ...
    onesr/data/meta_info/meta_info_Vimeo90K_train_GT.txt
...
class Vimeo90KRecurrentDataset(Vimeo90KDataset):
    ...
    def __getitem__(self, index):
        ...
        img_results = img2tensor(img_results)
        img_lqs = flow.stack(img_results[:7], dim=0)
        img_gts = flow.stack(img_results[7:], dim=0)

        if self.flip_sequence:  # flip the sequence: 7 frames to 14 frames
            img_lqs = flow.cat([img_lqs, img_lqs.flip(0)], dim=0)
            img_gts = flow.cat([img_gts, img_gts.flip(0)], dim=0)
