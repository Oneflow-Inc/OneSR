...
class DenseBlocksTemporalReduce(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = flow.cat((x[:, :, 1:-1, :, :], x1), 1)

        x2 = self.temporal_reduce2(x1)
        x2 = flow.cat((x1[:, :, 1:-1, :, :], x2), 1)

        x3 = self.temporal_reduce3(x2)
        x3 = flow.cat((x2[:, :, 1:-1, :, :], x3), 1)
    ...
    def __init__(
        ...
        for i in range(0, num_block):
            self.dense_blocks.append(
                nn.Sequential(
                    nn.BatchNorm3d(
                        num_feat + i * num_grow_ch, eps=eps, momentum=momentum
                    ),
                    nn.ReLU(inplace=True),
                    nn.Conv3d(
...
class DynamicUpsamplingFilter(nn.Module):
    ...
    def __init__(self, filter_size=(5, 5)):
        ...
        if not isinstance(filter_size, tuple):
            raise TypeError(
                f"The type of filter_size must be tuple, but got type{filter_size}"
            )
        if len(filter_size) != 2:
            raise ValueError(
                f"The length of filter size must be 2, but got {len(filter_size)}."
            )
        # generate a local expansion filter, similar to im2col
        self.filter_size = filter_size
        filter_prod = np.prod(filter_size)
        expansion_filter = flow.eye(int(filter_prod)).view(
            int(filter_prod), 1, *filter_size
        )  # (kh*kw, 1, kh, kw)
    ...
    def forward(self, x, filters):
        ...
        out = flow.matmul(expanded_input, filters)  # (n, h, w, 3, upsampling_square)
...
class DUF(nn.Module):
    ...
    def __init__(self, scale=4, num_layer=52, adapt_official_weights=False):
        ...
        self.dense_block1 = DenseBlocks(
            ...
            adapt_official_weights=adapt_official_weights,
        )  # T = 7
        ...
        self.conv3d2 = nn.Conv3d(
            channels, 256, (1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1), bias=True
        )

        self.conv3d_r1 = nn.Conv3d(
            256, 256, (1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0), bias=True
        )
        ...
        self.conv3d_f1 = nn.Conv3d(
            256, 512, (1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0), bias=True
        )
