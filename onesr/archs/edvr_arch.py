...
class PCDAlignment(nn.Module):
    ...
    def forward(self, nbr_feat_l, ref_feat_l):
        ...
        for i in range(3, 0, -1):
            level = f"l{i}"
            offset = flow.cat([nbr_feat_l[i - 1], ref_feat_l[i - 1]], dim=1)
            ...
            if i == 3:
                ...
                offset = self.lrelu(
                    self.offset_conv2[level](
                        flow.cat([offset, upsampled_offset], dim=1)
                    )
                )
            ...
            if i < 3:
                feat = self.feat_conv[level](flow.cat([feat, upsampled_feat], dim=1))
            if i > 1:
        ...
        # Cascading
        offset = flow.cat([feat, ref_feat_l[0]], dim=1)
        offset = self.lrelu(
...
class TSAFusion(nn.Module):
    ...
    def forward(self, aligned_feat):
        ...
        for i in range(t):
            ...
            corr = flow.sum(emb_neighbor * embedding_ref, 1)  # (b, h, w)
            corr_l.append(corr.unsqueeze(1))  # (b, 1, h, w)
        corr_prob = flow.sigmoid(flow.cat(corr_l, dim=1))  # (b, t, h, w)
        ...
        attn = self.lrelu(self.spatial_attn2(flow.cat([attn_max, attn_avg], dim=1)))
        # pyramid levels
        attn_level = self.lrelu(self.spatial_attn_l1(attn))
        attn_max = self.max_pool(attn_level)
        attn_avg = self.avg_pool(attn_level)
        attn_level = self.lrelu(
            self.spatial_attn_l2(flow.cat([attn_max, attn_avg], dim=1))
        )
...
class EDVR(nn.Module):
    ...
    def forward(self, x):
        ...
        aligned_feat = flow.stack(aligned_feat, dim=1)  # (b, t, c, h, w)
