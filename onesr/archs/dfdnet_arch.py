...
class DFDNet(nn.Module):
    ...
    def swap_feat(self, vgg_feat, updated_feat, dict_feat, location, part_name, f_size):
        ...
        part_feat = vgg_feat[
            :, :, location[1] : location[3], location[0] : location[2]
        ].clone()
        # resize original vgg features
        part_resize_feat = F.interpolate(
            part_feat, dict_feat.size()[2:4], mode="bilinear", align_corners=False
        )
        ...
        select_idx = flow.argmax(similarity_score)
        swap_feat = F.interpolate(
            dict_feat[select_idx : select_idx + 1], part_feat.size()[2:4]
        )
        # attention
        attn = self.attn_blocks[f"{part_name}_" + str(f_size)](swap_feat - part_feat)
        attn_feat = attn * swap_feat
        # update features
        updated_feat[:, :, location[1] : location[3], location[0] : location[2]] = (
            attn_feat + part_feat
        )
        return updated_feat
