...
from onesr.archs.arch_util import flow_warp
from onesr.archs.basicvsr_arch import ConvResidualBlocks
from onesr.archs.spynet_arch import SpyNet

# from onesr.ops.dcn import ModulatedDeformConvPack
...
class BasicVSRPlusPlus(nn.Module):
    ...
    def check_if_mirror_extended(self, lqs):
        ...
        if lqs.size(1) % 2 == 0:
            lqs_1, lqs_2 = flow.chunk(lqs, 2, dim=1)
            if flow.norm(lqs_1 - lqs_2.flip(1)) == 0:
    ...
    def propagate(self, feats, flows, module_name):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i > 0 and self.is_with_alignment:
                ...
                feat_n2 = flow.zeros_like(feat_prop)
                flow_n2 = flow.zeros_like(flow_n1)
                cond_n2 = flow.zeros_like(cond_n1)
                ...
                cond = flow.cat([cond_n1, feat_current, cond_n2], dim=1)
                feat_prop = flow.cat([feat_prop, feat_n2], dim=1)
    ...
    def upsample(self, lqs, feats):
        ...
        return flow.stack(outputs, dim=1)
...
#         extra_feat = flow.cat([extra_feat, flow_1, flow_2], dim=1)
#         out = self.conv_offset(extra_feat)
#         o1, o2, mask = flow.chunk(out, 3, dim=1)

#         # offset
#         offset = self.max_residue_magnitude * flow.tanh(flow.cat((o1, o2), dim=1))
#         offset_1, offset_2 = flow.chunk(offset, 2, dim=1)
#         offset_1 = offset_1 + flow_1.flip(1).repeat(1, offset_1.size(1) // 2, 1, 1)
#         offset_2 = offset_2 + flow_2.flip(1).repeat(1, offset_2.size(1) // 2, 1, 1)
#         offset = flow.cat([offset_1, offset_2], dim=1)

#         # mask
#         mask = flow.sigmoid(mask)
...
#     input = flow.rand(1, 2, 3, 64, 64).cuda()
#     output = model(input)
#     print('===================')
#     print(output.shape)
