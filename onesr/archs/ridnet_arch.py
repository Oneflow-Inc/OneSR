...
class MeanShift(nn.Conv2d):
    ...
    def __init__(self, rgb_range, rgb_mean, rgb_std, sign=-1, requires_grad=True):
        ...
        std = flow.Tensor(rgb_std)
        self.weight.data = flow.eye(3).view(3, 3, 1, 1)
        self.weight.data.div_(std.view(3, 1, 1, 1))
        self.bias.data = sign * rgb_range * flow.Tensor(rgb_mean)
...
class MergeRun(nn.Module):
    ...
    def forward(self, x):
        ...
        out = flow.cat([dilation1, dilation2], dim=1)
