...
class SeqConv3x3(nn.Module):
    ...
    def __init__(self, seq_type, in_channels, out_channels, depth_multiplier=1):
        ...
        if self.seq_type == "conv1x1-conv3x3":
            ...
            conv0 = flow.nn.Conv2d(
                self.in_channels, self.mid_planes, kernel_size=1, padding=0
            )
            self.k0 = conv0.weight
            self.b0 = conv0.bias

            conv1 = flow.nn.Conv2d(self.mid_planes, self.out_channels, kernel_size=3)
            ...
            # init scale and bias
            scale = flow.randn(size=(self.out_channels, 1, 1, 1)) * 1e-3
            self.scale = nn.Parameter(scale)
            bias = flow.randn(self.out_channels) * 1e-3
            bias = flow.reshape(bias, (self.out_channels,))
            self.bias = nn.Parameter(bias)
            # init mask
            self.mask = flow.zeros((self.out_channels, 1, 3, 3), dtype=flow.float32)
            ...
            # init scale and bias
            scale = flow.randn(size=(self.out_channels, 1, 1, 1)) * 1e-3
            self.scale = nn.Parameter(flow.FloatTensor(scale))
            bias = flow.randn(self.out_channels) * 1e-3
            bias = flow.reshape(bias, (self.out_channels,))
            self.bias = nn.Parameter(flow.FloatTensor(bias))
            # init mask
            self.mask = flow.zeros((self.out_channels, 1, 3, 3), dtype=flow.float32)
            for i in range(self.out_channels):
                self.mask[i, 0, 0, 0] = 1.0
    ...
    def rep_params(self):
        ...
        if self.seq_type == "conv1x1-conv3x3":
            ...
            # re-param conv bias
            rep_bias = flow.ones(
                1, self.mid_planes, 3, 3, device=device
            ) * self.b0.view(1, -1, 1, 1)
            rep_bias = F.conv2d(input=rep_bias, weight=self.k1).view(-1,) + self.b1
        else:
            tmp = self.scale * self.mask
            k1 = flow.zeros((self.out_channels, self.out_channels, 3, 3), device=device)
            for i in range(self.out_channels):
                k1[i, i, :, :] = tmp[i, 0, :, :]
            b1 = self.bias
            # re-param conv kernel
            ...
            # re-param conv bias
            rep_bias = flow.ones(
                1, self.out_channels, 3, 3, device=device
            ) * self.b0.view(1, -1, 1, 1)
...
class ECB(nn.Module):
    ...
    def __init__(
        ...
        self.conv1x1_sbx = SeqConv3x3(
            "conv1x1-sobelx", self.in_channels, self.out_channels
        )
        self.conv1x1_sby = SeqConv3x3(
            "conv1x1-sobely", self.in_channels, self.out_channels
        )
        self.conv1x1_lpl = SeqConv3x3(
            "conv1x1-laplacian", self.in_channels, self.out_channels
        )
...
class ECBSR(nn.Module):
    ...
    def forward(self, x):
        if self.num_in_ch > 1:
            shortcut = flow.repeat_interleave(x, self.scale * self.scale, dim=1)
        else:
