...
class BasicVSR(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(n - 1, -1, -1):
            ...
            feat_prop = oneflow.cat([x_i, feat_prop], dim=1)
            feat_prop = self.backward_trunk(feat_prop)
            out_l.insert(0, feat_prop)

        # forward branch
        feat_prop = oneflow.zeros_like(feat_prop)
        for i in range(0, n):
            x_i = x[:, i, :, :, :]
            if i > 0:
            ...
            # upsample
            out = oneflow.cat([out_l[i], feat_prop], dim=1)
        ...
        return oneflow.stack(out_l, dim=1)
    ...
    def forward(self, x):
        ...
        for i in range(n - 1, -1, -1):
            ...
            if i in keyframe_idx:
                feat_prop = oneflow.cat([feat_prop, feats_keyframe[i]], dim=1)
        ...
        for i in range(0, n):
            ...
            feat_prop = oneflow.cat([x_i, out_l[i], feat_prop], dim=1)
            feat_prop = self.forward_trunk(feat_prop)
        ...
        return oneflow.stack(out_l, dim=1)[..., : 4 * h_input, : 4 * w_input]
...
class EDVRFeatureExtractor(nn.Module):
    ...
    def __init__(self, num_input_frame, num_feat, load_path):
        ...
        if load_path:
            self.load_state_dict(
                oneflow.load(load_path, map_location=lambda storage, loc: storage)[
                    "params"
                ]
            )
    ...
    def forward(self, x):
        ...
        aligned_feat = oneflow.stack(aligned_feat, dim=1)  # (b, t, c, h, w)
