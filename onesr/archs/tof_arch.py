    ...
    def __init__(self, load_path=None):
        ...
        if load_path:
            self.load_state_dict(
                flow.load(load_path, map_location=lambda storage, loc: storage)[
                    "params"
                ]
            )
...
class TOFlow(nn.Module):
    ...
    def __init__(self, adapt_official_weights=False):
        ...
        self.register_buffer(
            "mean", flow.Tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)
        )
        self.register_buffer("std", flow.Tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1))
    ...
    def forward(self, lrs):
        ...
        # reconstruction
        hr = flow.stack(lr_aligned, dim=1)
